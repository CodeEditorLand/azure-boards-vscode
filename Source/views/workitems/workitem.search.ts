...
import type * as DevOpsClient from "azure-devops-node-api";
import type { IHttpClientResponse } from "azure-devops-node-api/interfaces/common/VsoBaseInterfaces";

export class SearchProvider {
	private _searchUrl = "https://almsearch.dev.azure.com/";
	...
	async searchWorkItems(data: object): Promise<WorkItem[]> {
		...
		const res: IHttpClientResponse = await client.post(
			this._searchUrl,
			JSON.stringify(data),
			{
		...
		const jsonObject: ISearchRoot = JSON.parse(body);
